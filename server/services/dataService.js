...
const formatCalendarData = (shiftsData) => {
  ...
  shiftsData.forEach((shift) => {
    const obj = {};
    const { enterTime, exitTime } = shift;
    const type = shiftService.getShiftType(enterTime);
    obj.Id = shift._id;
    obj.Subject = `${type} Shift`;

    const [enterHours, enterMinutes] = enterTime.split(":");
    const [exitHours, exitMinutes] = exitTime.split(":");
    const enterDate = new Date(shift.date);
    const exitDate = new Date(shift.date);
    enterDate.setHours(parseInt(enterHours, 10));
    enterDate.setMinutes(parseInt(enterMinutes, 10));
    exitDate.setHours(parseInt(exitHours, 10));
    exitDate.setMinutes(parseInt(exitMinutes, 10));

    obj.EndTime = exitDate;
    obj.StartTime = enterDate;
    obj.IsReadonly = true;
    obj.subjectColor = "#03C9D7";
